...
def train_sup(args, model, optimizer, train_loader, val_loader, criterion, max_iters, print_iters, eval_iters):
    ...
    for epoch in range(200):
        ...
        if rank == 0:
            ...
            # Check if the new mIoU is better than the best
            ...
            for idx, (class_idx, iou) in enumerate(val_score['Class IoU'].items()):           
                logging.info('Class {} IoU: {:.5f}'.format(class_idx, iou))
                print('Class {} IoU: {:.5f}'.format(class_idx, iou))
            model.train()
    ...
    for epoch in range(200):
        ...
        if rank == 0:
            ...
            if miou > best_miou:
                ...
                best_epoch = epoch
                # Save the model weights if it's the best mIoU
                torch.save(model.state_dict(), "FloodNet_Best_weights.pth")
                logging.info("New best mIoU found. Model weights saved.")
                print("New best mIoU found. Model weights saved.")
...
def val(model, data_loader):
    ...
    score = eval_seg.scores(gts, preds, num_classes=10)
    model.train()
    return score
...
def main():
    ...
    if mit_type == '5':
        args.config = 'local_configs/segformer/B' + mit_type + '/segformer.b' + mit_type + '.640x640.floodnet.160k.py'
    else:
        args.config = 'local_configs/segformer/B' + mit_type + '/segformer.b' + mit_type + '.512x512.floodnet.160k.py'
