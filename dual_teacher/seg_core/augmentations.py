...
def class_mix(occluder_mask, occluder, occludee):
    """
    Mix occluder and occludee tensors using a binary mask.
    Supports both RGB images and softmax class probabilities.

    Args:
        occluder_mask (Tensor): [B, H, W]
        occluder (Tensor): [B, C, H, W]
        occludee (Tensor): [B, C, H, W]

    Returns:
        Tensor: class-mixed output of shape [B, C, H, W]
    """
    assert occluder.shape == occludee.shape, \
        f"Shape mismatch: occluder {occluder.shape}, occludee {occludee.shape}"

    # Automatically repeat mask to match number of channels
    B, C, H, W = occluder.shape
    occluder_mask = occluder_mask.unsqueeze(1).repeat(1, C, 1, 1)
