...
class rescue_dset(BaseDataset):
    def __init__(self, data_root, data_list, trs_form):
        super(rescue_dset, self).__init__(data_list)
...
def build_rescueloader(split, all_cfg, seed=0):
    ...
    dset = rescue_dset(cfg["data_root"], cfg["data_list"], trs_form)
...
def build_rescue_semi_loader(split, all_cfg, seed=0):
    cfg_dset = all_cfg["dataset"]
    cfg = copy.deepcopy(cfg_dset)
    cfg.update(cfg.get(split, {}))
